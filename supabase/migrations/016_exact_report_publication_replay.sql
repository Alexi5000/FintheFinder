...
declare
  ...
  existing_report public.research_reports%rowtype;
  existing_final_audit public.research_audits%rowtype;
begin
  ...
  if completed_run.id is not null then
    ...
    where report.session_id = p_session_id
      and report.id = report_id;

    select audit.*
    into existing_final_audit
    from public.research_audits audit
    where audit.session_id = p_session_id
      and audit.run_id = p_run_id
      and audit.audit_type = 'final_review'
      and audit.ok is true
    order by audit.created_at desc, audit.id desc
    limit 1;

    if existing_report.id is null or existing_final_audit.id is null then
      raise exception 'report publication replay is missing committed report or audit' using errcode = '55000';
    end if;

    if existing_report.title is distinct from p_report ->> 'title'
      or existing_report.executive_summary is distinct from p_report ->> 'executive_summary'
      or existing_report.sections is distinct from p_report -> 'sections'
      or existing_report.citations is distinct from p_report -> 'citations'
      or existing_report.markdown is distinct from p_report ->> 'markdown'
      or existing_report.created_at is distinct from (p_report ->> 'created_at')::timestamptz
      or existing_final_audit.ok is distinct from true
      or existing_final_audit.issues is distinct from coalesce(p_final_audit -> 'issues', '[]'::jsonb) then
      raise exception 'report publication replay payload does not match committed report' using errcode = '23505';
    end if;
