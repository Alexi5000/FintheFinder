...
Run events are append-only within the session lifecycle. Supabase rejects post-insert updates to event identity, session, attempt, phase, type, severity, actor, step, message, timing, trace/correlation IDs, metadata, and created timestamp, and rejects direct event deletes. The only permitted event mutations are foreign-key cleanup paths: `run_id` can become `null` when a referenced run is deleted, and events can be removed by parent session cascade.
...
Report publication uses the service-role-only `publish_research_report_for_attempt` RPC. It validates the report and final-review audit payloads, returns exact committed replays as idempotent success, rechecks critical gaps under database locks, and then either returns the run/session to `awaiting_approval` or commits the final audit, `research_reports` row, `report_ready` event, session completion, run completion, attempt completion, and `research_job_leases` cleanup in one transaction. Non-test publication requires an attempt-fenced worker context (`runId`, `attemptId`, and `workerId`) before any report table writes. A `report publication replay payload does not match committed report` error means a completed worker retry submitted a different report or final-audit payload for the same fenced attempt; treat it as an integrity incident and inspect the worker logs, trace ID, and committed report before retrying. A late critical-gap block is a policy gate, not an outage; it should be investigated from the approval/gap history rather than from post-mortems.

Session, approval, and memory mutations must go through the hosted API routes. Direct authenticated Supabase writes to `research_sessions`, `research_approvals`, and `research_memories` are intentionally denied so rate limits, ownership checks, state-machine rules, critical-gap blocking, waiver notes, memory size limits, and secret-like memory scanning stay inside audited server code. The approval route delegates decisions to the service-role-only `record_research_approval_decision` RPC, which locks the owned session, rejects decisions unless it is currently `awaiting_approval`, and commits gap waivers, approval history, events, state changes, and follow-on run enqueueing transactionally.
