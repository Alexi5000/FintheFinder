...
import { getApprovalsForUser } from '@/server/research/repository';
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!user) return apiError('unauthorized', 'Sign in to view approvals.', 401);
    const { id } = await context.params;
    return NextResponse.json({ approvals: await getApprovalsForUser(user.id, id) });
  } catch (error) {
