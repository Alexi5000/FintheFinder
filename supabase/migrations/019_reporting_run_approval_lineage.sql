...
declare
  ...
  source_research_run public.research_runs%rowtype;
...
begin
  ...
  select *
  into source_research_run
  from public.research_runs
  where session_id = p_session_id
    and status in ('awaiting_approval','completed')
    and coalesce(metadata ->> 'stage', 'research') <> 'reporting'
  ...
  if p_action = 'approve' then
    if source_research_run.id is null then
      return jsonb_build_object(
        'ok', false,
        'error', jsonb_build_object('code', 'approval_not_available', 'details', jsonb_build_object('reason', 'source_research_run_missing'))
      );
    end if;
    ...
    values (
      ...
      case
        when p_action = 'approve' then jsonb_build_object(
          ...
          'sourceResearchRunId', source_research_run.id,
