...
- Worker runtime config parsing, no-work paths, lease heartbeats, lease-guarded pipeline persistence, lease-loss terminal-write blocking, reporting dispatch, sanitized failure artifacts, and best-effort run-summary memory
- Supabase migration parity for table columns, SQL/Zod enum checks, event constraints, RLS, cross-session graph integrity, service-role RPCs, expired-lease heartbeat rejection, API-only approval writes, and repository artifact/event payload shape
