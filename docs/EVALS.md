...
`npm run evals:live` is fail-closed. It requires OpenAI, Exa, and Supabase environment variables plus `docs/demo/live-demo.json` generated by `npm run demo:export -- --reporting-run-id <id> --media <path>`. The manifest must point at an eval output artifact from the configured reporting run with at least three expected-vs-actual live scenarios: citation integrity, claim traceability, and safety language. It must also include session-level run export evidence for the research run, approval, reporting run, final audit, per-stage costs, and aggregate usage. `npm run demo:record` must pass against the same manifest before the live proof can be treated as recorded evidence. Missing credentials, exporter provenance, Supabase evidence, reporting-run lineage, or local artifacts should fail the command instead of silently passing offline fixtures.

Live eval proof is not persisted by `npm run evals:persist`; it remains tied to `docs/demo/live-demo.json`, the referenced eval output artifact, Supabase re-fetch validation, and `npm run evals:live`. The live command prints the session ID, research run ID, reporting run ID, approval ID, reporting trace ID, manifest SHA-256, scenario count, issues/regressions arrays, and the validated cost object.
